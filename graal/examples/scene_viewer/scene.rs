...
impl Scene {
    ...
    pub fn start_upload<'a>(&'a mut self, queue: &'a mut Queue) -> SceneUploader<'a> {
        SceneUploader {
            scene: self,
...
impl<'a> SceneUploader<'a> {
    ...
    fn load_mesh_from_obj(&mut self, queue: &mut Queue, model: &tobj::Model) -> MeshId {
        ...
        let mesh_data = MeshData {
            ...
            index_count,
            vertex_count,
            bounds: BoundingBox { min: p_min, max: p_max },
        };
    ...
    pub fn finish(self) {
        ...
        self.frame.add_transfer_pass("upload scene data", false, |pass| {
            // register accesses
